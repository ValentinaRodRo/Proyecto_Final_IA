...
class Hello(Policy):
    ...
    def act(self, board: np.ndarray) -> int:
        ...
        acc_win = self.accion_ganadora(state)
        if acc_win is not None:
            self.estado_anterior = estado
            self.accion_anterior = acc_win
            return acc_win

        # 4.bloqueo inmediato
        acc_block = self.accion_bloqueo(state)
        if acc_block is not None:
            self.estado_anterior = estado
            self.accion_anterior = acc_block
            return acc_block

        #5. IMPLEMENTACIÓN DEL MCTS AHORA DE MANERA FORZADA

        #Ajustamos las simulaciones segun el modo
        simulaciones_mcts = 80 if self.modo_torneo else 80

        # Se ejecuta MCTS
        valores_mcts = self.mcts(state, simulaciones=simulaciones_mcts)

        #Explotacion Pura del MCTS (maximo valor), 
        #se ignora el epsilon-greedy y la Q-table para la decision final
